...
try:
    from core.job_enricher.enrich_job_data import enrich_field, enrich_job_data
except ImportError:
    # Define the functions locally if import fails
    logger = get_logger()
    
    def enrich_job_data(job_data):
        """Enriches all supported fields in the job data"""
        logger.info("Enriching all supported job fields (local implementation)")
    ...
    def enrich_field(job_data, field_name):
        ...
        logger.info(f"Enriching job field: {field_name} (local implementation)")
...
# CORS headers are now handled globally in the main app
...
def job_enricher_endpoint():
    ...
    try:
        ...
        # Use the core function to enrich all fields
